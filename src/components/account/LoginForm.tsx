...
const LoginForm = ({ onSubmit, defaultValues = {} }: Props) => {
	...
	const [loginUser, { isLoading }, errorMessage] = useLoginUser();
	...
	return (
		<FocusTrap active>
			<div
				className={`${styles.main} bg-greybase w-2/5 rounded-3xl`}
				style={{
					boxShadow:
			...
			>
				...
				>
					...
					<Group position="center" mb="40px">
						<Text c="dimmed" fz="lg" className={styles.texts}>
							Нет еще аккаунта?{" "}
							<Link href="/account/registration/" className=" text-blue-500">
								Создать аккаунт
							</Link>{" "}
						</Text>
						<Link href="/account/resetpassword/" className=" text-blue-500">
							Забыли пароль?
						</Link>
					</Group>
				</form>
