	...
	return (
		<div className={styles.mainDetail}>
			...
			>
				...
				/>
				<button
					className={`  text-bluelogin ${styles.rating_btn}`}
				...
				{ratingOpen ? (
					<div className="flex mt-2">
						<Rating onChange={setValue} />
						<button
							className={` text-bluelogin ml-2 text-xs ${styles.rating_btn}`}
							onClick={() => {
				...
				<Text mt="xs" size="sm" className={` ${styles.text} flex items-end`}>
					...
					<a href={item.social_net}>{item.social_net}</a>
				</Text>
			</Card>
		</div>
	);
